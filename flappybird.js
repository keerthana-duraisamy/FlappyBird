...
// physics
let velocityX = -2; //pipesmoving left speed
...
let bgm = new Audio("./super_mario_theme.mp3");
bgm.loop = true;
...
window.onload = function () {
    ...
    setInterval(animateBird, 100); //every 1/10 seconds
...
function update() {
    ...
    for (let i = 0; i < pipeArray.length; i++) {
        ...
        if (!pipe.passed && bird.x > pipe.x + pipe.width) {
            score += 0.5; //0.5 because there are 2 pipes so 0.5*2 =1, 1for each set of pipes
            pipe.passed = true;
        }
...
function placePipes() {
    ...
    //(0-1) * pipeHeight/2
    //0 -> -128(pipeHeight/4)
    //1 -> -128 - 256(pipeHeight/4 - pipeHeight/2) = -3/4 pipeHeigth
